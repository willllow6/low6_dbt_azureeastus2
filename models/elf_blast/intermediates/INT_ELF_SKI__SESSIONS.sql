...
time_diff_flags as (
    ...
    select
        *,
        case
            when lag(entry_starts_at_utc) over (partition by sso_user_id order by entry_id) is null
                then 1
            when timediff(second, lag(entry_starts_at_utc) over (partition by sso_user_id order by entry_id),entry_starts_at_utc) > 1800
                then 1
            else 0
...
time_diff_partitions as (
    ...
    select
        *,
        sum(time_diff_flag) over (partition by sso_user_id order by entry_id rows between unbounded preceding and current row) as time_diff_partition
...
session_ids as (
    ...
    select
        *,
        first_value(entry_id) over (partition by sso_user_id, time_diff_partition order by entry_id) as session_id
        -- last_value(level_name) over (partition by sso_user_id, time_diff_partition order by entry_id) as session_abandoned_level
...
user_sessions as (
    ...
    select 
        ...
        max(entry_ends_at_utc) as session_ends_at_utc,
        case 
